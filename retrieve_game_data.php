...
$data = [];
//full data attempt
$game_query = "SELECT a.full_name AS full_name_a, a.abbr_name AS abbr_name_a, a.logo_src AS logo_src_a, h.full_name AS full_name_h, h.abbr_name AS abbr_name_h, h.logo_src AS logo_src_h, g.home_spread, g.spread_odds_a, g.spread_odds_h, g.moneyline_odds_a, g.moneyline_odds_h, g.overunder_points, g.overunder_odds_o, g.overunder_odds_u, g.game_time FROM `games` AS g JOIN `teams` AS h ON g.team_a_id = h.ID JOIN `teams` AS a ON g.team_H_id = a.ID WHERE g.league_id = 1 ORDER BY g.game_time";
